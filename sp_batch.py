import logging, datetime, time, os, argparse
...
"""
# bmiller, aug 2022
# the sp_creds.py file (not in repo) must contain one or more StreetPerfect creds in a dictionary
...
creds = {
	...
	'aws' : {
		'api_id' : 'user@postmedia.com',
		'api_key' : 'api-key',
		'api_url' : 'https://stage.streetperfect-dev.com/api/',
		},
	'prod' : {
		'api_id' : 'user@postmedia.com',
		'api_key' : 'api-key',
...
def sp_batch(sp_api_id, sp_api_key, sp_url, cfg, input_file, out_good_recs, out_bad_recs, verify_ssl=True):
    ...
    try:
        ...
        if r.status == "InputReady":
            ...
            #todo, make this more robust so it doesn't poll forever
            while 1:
                # now wait till its done
                # possible statuses are: Unknown, Error, Empty, Starting, Running, Stopping, Stopped, InputReady, OutputReady
                r = client.caBatchStatus()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(prog='sp_batch', 
                                     ...
                                     description='Executes a StreetPerfect batch validation process against a passed list of addresses.')
    ...
    parser.add_argument('-spCredKey', default='aws', help='if using the sp_creds.py file, specify the creds key')
    ...
    if args.spApiId and args.spApiKey:
        ...
        try:
            import sp_creds
            creds = sp_creds.creds[args.spCredKey]
            sp_api_id = creds['api_id']
            sp_api_key = creds['api_key']
            sp_url = creds['api_url']
            print('read sp_creds.py')
        except Exception as e:
            print(f'sp_creds.py read error: {e}')
